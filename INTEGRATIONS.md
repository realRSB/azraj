...
Azraj's integrations are provided by [Composio](https://composio.dev/?utm_source=chris&utm_medium=youtube&utm_campaign=collab), a tool-aggregator that exposes 1000+ third-party services (Gmail, GitHub, Slack, Notion, Linear, Google Drive, HubSpot, Salesforce, …) behind one API.
...
## How it hooks into Azraj

Each connected Composio toolkit is registered in Azraj's integration registry (`server/integrations/registry.ts`) keyed by its slug. When the dispatcher calls:
...
Azraj does not store third-party passwords or OAuth tokens for this feature. Login state lives in the selected local Chrome profile.
...
When you click Connect on one of those, Azraj surfaces an amber banner explaining that you need to:
...
This is a one-time setup per toolkit (not per user) — all users of your Azraj instance reuse the same auth config after that.

## Notes

- **Single-tenant by default.** All connections are keyed under `COMPOSIO_USER_ID` (defaults to `boop-default`). Override if you manage Composio sessions elsewhere and want Azraj to share that user.
...
- **No tokens live in Azraj.** Composio stores OAuth credentials on their side. Azraj never sees them.
