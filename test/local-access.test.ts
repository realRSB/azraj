...
describe("local server access", () => {
  ...
  it("exposes the public site, health, provider webhooks, and public phone auth routes", () => {
    ...
    expect(
      isPublicServerRequest(request({ method: "POST", url: "/public-auth/start" })),
    ).toBe(true);
    expect(
    ...
    expect(
      isPublicServerRequest(request({ method: "POST", url: "/api/public-auth/join/start" })),
    ).toBe(true);
    expect(
      isPublicServerRequest(request({ method: "GET", url: "/api/public-auth/config" })),
    ).toBe(true);
    expect(
